...
function isStudent(kid) {
  return kid === 'student';
}
function isTeacher(role) {
  return role === 'teacher';
}
function isAdmin(ctrl) {
  return ctrl === 'admin';
}
function isElementary(school) {
  return school === 'elementary';
}
function areDifferentPeople(strg1, strg2) {
  return strg1 !== strg2;
}
function isMiddleSchoolTeacher(admin, num) {
  return admin === 'teacher' && num >= 6 && num <= 8;
}
function notAnElementarySchoolAdministrator(strg1, strg2 ) {
  return strg1 !=='elementary' || strg2 !== 'admin';
}
